...
class ChannelDM implements PapuChannel {
  ...
  void endOfSample() {
    ...
    if (playLengthCounter > 0) {
      ...
      if (playLengthCounter == 0) {
        ...
        if (playMode == MODE_IRQ) irqGenerated = true;
      }
    }
  }
  ...
  void nextSample() {
    ...
    playLengthCounter--;
    playAddress++;
    if (playAddress > 0xFFFF) playAddress = 0x8000;
  ...
  void writeReg(int address, int value) {
    if (address == 0x4010) {
      ...
      if ((value >> 6) == 0)
        ...
        playMode = MODE_LOOP;
      else if ((value >> 6) == 2) playMode = MODE_IRQ;

      if ((value & 0x80) == 0) irqGenerated = false;
  ...
  void setEnabled(bool value) {
    if ((!_isEnabled) && value) playLengthCounter = playLength;
    _isEnabled = value;
  }
