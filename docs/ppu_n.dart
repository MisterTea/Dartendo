...
class PPU {
  ...
  late Int32List _vramMirrorTable; // Mirroring Lookup Table.
  ...
  // Sprite data:
  late List<int> sprX; // X coordinate
  late List<int> sprY; // Y coordinate
  late List<int> sprTile; // Tile Index (into pattern table)
  late List<int> sprCol; // Upper two bits of color
  late List<bool> vertFlip; // Vertical Flip
  late List<bool> horiFlip; // Horizontal Flip
  late List<bool> bgPriority; // Background priority
  ...
  // Name table data:
  late List<int> _ntable1;
  late List<NameTable> _nameTable;
  int _currentMirroring = -1;

  // Palette data:
  late List<int> _sprPalette;
  late List<int> _imgPalette;
  ...
  late List<Tile?> _scantile;
  late List<int> _attrib;
  late List<int> _bgbuffer;
  late List<int> _pixrendered;
  late List<int> _spr0dummybuffer;
  late List<int> _dummyPixPriTable;
  late List<int> _oldFrame;
  late List<bool> _scanlineChanged;
  //Tile _t;

  late List<int> _bgColor_table;

  int cycles = 0;

  PPU(this.nes) {
    // Build the bgColor lookup table.
    _bgColor_table = List<int>.filled(5, 0);
    ...
    _scantile = List<Tile?>.filled(32, null);
    _attrib = Util.newIntList(32, 0);
    _bgbuffer = Util.newUInt8List(256 * 240, 0);
    _pixrendered = Util.newIntList(256 * 240, 0);
    _spr0dummybuffer = Util.newUInt8List(256 * 240, 0);
    _dummyPixPriTable = Util.newUInt8List(256 * 240, 0);
    _oldFrame = Util.newUInt8List(256 * 240, 0);
    _scanlineChanged = Util.newBoolList(240, false);

    buffer = Util.newUInt8List(256 * 240, 0);
  }
  ...
  void init() {
    ...
    _nameTable = [];
    for (int i = 0; i < 4; i++) {
      _nameTable.add(NameTable(32, 32, "Nt$i"));
    }
  ...
  void setMirroring(int mirroring) {
    if (mirroring == _currentMirroring) return;

    _currentMirroring = mirroring;
    triggerRendering();

    // Remove mirroring:
    if (_vramMirrorTable == null) _vramMirrorTable = Util.newIntList(0x8000, 0);
    for (int i = 0; i < 0x8000; i++) _vramMirrorTable[i] = i;
    ...
    if (scanline < 19 + _vblankAdd) {
      ...
      if (f_bgVisibility == 1) {
        ...
        if (!hitSpr0 && f_spVisibility == 1) {
          if (sprX[0] >= -7 &&
              sprX[0] < 256 &&
              sprY[0] + 1 <= (scanline - _vblankAdd + 1 - 21) &&
              (sprY[0] + 1 + (f_spriteSize == 0 ? 8 : 16)) >=
                  (scanline - _vblankAdd + 1 - 21)) {
  ...
  void startFrame() {
    ...
    if (f_dispType == 0) {
      ...
      if (f_color < _bgColor_table.length) bgColor = _bgColor_table[f_color];
    }

    buffer.fillRange(0, buffer.length, bgColor);
    _pixrendered.fillRange(0, _pixrendered.length, 65);
  }
  ...
  void endFrame() {
    ...
    if (/*_clipTVcolumn ||*/ f_bgClipping == 0 || f_spClipping == 0) {
      ...
      for (int y = 0; y < 240; y++) {
        for (int x = 0; x < 8; x++) buffer[(y << 8) + x] = 0;
      }
    }
  ...
  void updateControlReg2(int value) {
    ...
    if (f_dispType == 0) nes.palTable.setEmphasis(f_color);
    updatePalettes();
  }
  ...
  void writeVRAMAddress(int address) {
    ...
    // Invoke mapper latch:
    cntsToAddress();
    if (_vramAddress < 0x2000) nes.memMapper!.latchAccess(_vramAddress);
  }
  ...
  void triggerRendering() {
    if (scanline - _vblankAdd >= 21 && scanline - _vblankAdd <= 260) {
      ...
      renderFramePartially(buffer, lastRenderedScanline + 1,
          scanline - _vblankAdd - 21 - lastRenderedScanline);
      ...
      for (int tile = 0; tile < 32; ++tile) {
        if (scan >= 0) {
          ...
          if (_validTileData) {
            ...
            // Fetch data:
            t = ptTile[
                baseTile + _nameTable[curNt].getTileIndex(_cntHT, _cntVT)];
            tpix = t.pix;
    ...
    for (int i = 0; i < 64; i++) {
      if (bgPriority[i] == bgPri &&
        ...
        if (f_spriteSize == 0) {
          ...
          final int tileIndex =
              (f_spPatternTable == 0 ? sprTile[i] : sprTile[i] + 256);
  ...
  void patternWrite(int address, Object value,
    ...
    if (value is int) {
      ...
      if (leftOver < 8) {
        ptTile[tileIndex]
            .setScanline(leftOver, value, ppuMem.load(address + 8));
      } else {
        ptTile[tileIndex]
            .setScanline(leftOver - 8, ppuMem.load(address - 8), value);
      }
    } else {
      ...
      for (int i = 0; i < length; i++) {
        ...
        if (leftOver < 8) {
          ptTile[tileIndex].setScanline(
              leftOver, valueList[offset + i], ppuMem.load(address + 8 + i));
        } else {
          ptTile[tileIndex].setScanline(leftOver - 8,
              ppuMem.load(address - 8 + i), valueList[offset + i]);
        }
      }
    }
  }
