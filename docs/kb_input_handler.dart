...
class KbInputHandler {
  ...
  List<bool> allKeysState = Util.newBoolList(255, false);
  List<int> keyMapping = Util.newIntList(KbInputHandler.NUM_KEYS, 0);
  int id;
  NES nes;

  KbInputHandler(this.nes, this.id) {
  ...
  void setKeyState(int padKey, int state) {
    ...
    else if (state == 0x40) allKeysState[keyMapping[padKey]] = false;
  }
  ...
  void keyPressed(KeyboardEvent ke) {
    int kc = ke.keyCode;
    if (kc >= allKeysState.length) return;
    ...
    if (id == 0) {
      switch (ke.code) {
        ...
        case 'F10':
          ...
          if (nes.rom != null) nes.rom!.closeRom();
          break;
