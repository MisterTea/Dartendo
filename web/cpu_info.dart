...
class OpData {
  ...
  OpData(this.inst, this.op, this.addr, this.size, this.cycles);
}
...
class CpuInfo {
  ...
  static void initOpData() {
    ...
    for (int i = 0; i < 256; i++) {
      opdata!.add(OpData(0xFF, 0, 0, 0, 0));
    }
  ...
  static void setOp(int inst, int op, int addr, int size, int cycles) {
    opdata![op] = OpData(inst, op, addr, size, cycles);
  }

  static void initInstNames() {
    instname = List<String>.filled(56, "");
